...
def train_absX(df_train, data_loader, model, loss_function, prev_x, scalers, collect, optimizer):
    ...
    furthermore, is this the portion of the code to implement a softmax gating function and apply
    the MoE approach?

    would this approach be then to run 2 LSTM and apply the gating funciton with its own set of
    trainable weights? 
    '''
